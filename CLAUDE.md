...
| 7c | **Paper-faithful PPO**: PPO-OR / PPO-MR / MT-PPO / GRPO-OR / GRPO-MR under the paper's own reward design | `docs/phase-7c-paper-faithful-ppo.md` | **Done** (2026-07-30) — all five arms trained at seed 42/500 steps and evaluated on the full 7,404-row held-out set. **Two arms collapsed**: `ppo` and `grpo_or`, both binary-outcome-only, into verified zero-gradient absorbing states (`grpo_or`: `grad_norm` exactly 0.0000 for 300 steps after step 184; `ppo`: format 0.003 with the *highest* retrieval of any arm — it learned to search and never answer). The three merged-reward arms trained cleanly: `ppo_mr` EM 0.301, `grpo_mr` 0.295, `mt_ppo` 0.274. **The paper's own MT-PPO-over-PPO-MR claim did not reproduce** (−0.027 EM vs the paper's +0.017) but is reported as *unresolved, not refuted* — n=1 vs the paper's n=5, and eval was shown bit-reproducible so all uncertainty is seed variance. See §13 (results) and §14 (handoff), especially **§14b: the paper uses Qwen2.5-7B, ~8.75x this repo's model** — its untrained baseline scores EM 0.160-0.292, i.e. about where our best trained arm lands. Also fixed a real bug: `evaluate_ppo.py` never recorded format compliance (§14c) |
