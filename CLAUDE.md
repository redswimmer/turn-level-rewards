...
**The paper uses `Qwen2.5-7B` — ~8.75x larger. This is the single biggest deviation in the repo
and was not recorded here until Phase 7c (2026-07-30).** Verified by reading arXiv:2505.11821v2
§6.2 directly: *"We use Qwen2.5-7B as the base model, E5 as the retriever… number of retrieved
passages to 3, and the maximum number of turns N_max to 4."* (Of those, `topk=3` and `N_max=4`
both **match** this repo; the retriever does not — this repo uses BM25, see "Why this design".)

Why it dominates every comparison against the paper's numbers: its own Table 2 reports the
**untrained** baselines on HotpotQA as EM **0.160** (Qwen2.5-7B-Base) and **0.292**
(Qwen2.5-7B-Instruct). Phase 7c's best trained arm reached EM **0.301** — i.e. **this repo's
fully-trained 0.8B model lands roughly where the paper's model starts.** Read every "we undershoot
the anchor" observation against that before attributing it to batch size, step count, or a bug.

It also drives the collapse failure mode. The trigger is the probability a batch contains zero
correct answers, `(1-p)^N`: model size lowers `p`, batch size lowers `N`, and they compound in the
same exponent. See `docs/phase-7c-paper-faithful-ppo.md` §14b.
...
| 7b | Full PPO/MT-PPO training runs + evaluation + comparison + matplotlib visuals | `docs/phase-7b-full-ppo-runs.md` | **Ran, then invalidated — results discarded, superseded by 7c.** It measured a defective trainer (rollout loop denied the final answer turn, no advantage normalization, no gradient clipping, inverted KL sign) against a reward that was not the paper's; see `docs/phase-7c-paper-faithful-ppo.md` §1. Its two salvageable deliverables *did* ship, in 7c: the matplotlib visuals (`scripts/plot_phase7c.py` → `results/phase7c_*.png`) and the README results section. Nothing here is outstanding — do not "finish" this phase, read 7c instead |
| 7c | **Paper-faithful PPO**: PPO-OR / PPO-MR / MT-PPO / GRPO-OR / GRPO-MR under the paper's own reward design | `docs/phase-7c-paper-faithful-ppo.md` | **Done** (2026-07-31) — six arms trained at seed 42/500 steps, all evaluated on the full 7,404-row held-out set. **Read §15 first; §13b is superseded.** **The paper's Eq. 9 claim reproduces**: PPO-MR → MT-PPO = **+3.90 EM** (paper +1.7), format 0.642→0.830. An earlier "did not reproduce" reading came from comparing against `ppo_mr`, which is NOT the paper's PPO-MR (it carries MT-PPO's own R^I) — the faithful baseline is `ppo_mr_paper`. **Two arms collapsed**, `ppo` and `grpo_or`, both binary-outcome-only, into verified zero-gradient absorbing states (`grpo_or`: grad_norm exactly 0.0000 for 300 steps, held-out tool-call frequency 0.0; `ppo`: format 0.003 with the *highest* retrieval of any arm — it learned to search and never answer). **Repo's own contribution**: keeping the deviating `ppo_mr` decomposes the paper's PPO-MR→MT-PPO step into a reward-**content** effect (+6.59 EM) and a turn-**placement** effect (−2.69 EM), which the paper's design cannot separate. Also: **λs=0 did NOT cause uncontrolled search** (contra §5.2), isolating the dead gradient as what killed `ppo`; and see **§14b — the paper uses Qwen2.5-7B, ~8.75x this repo's model**, whose *untrained* baseline (EM 0.160-0.292) is about where our best trained arm lands. Fixed a real bug: `evaluate_ppo.py` never recorded format compliance (§14c) |
...
| 8 | LLM-as-judge reward (Bedrock + gpt-oss): full MT-PPO run with judge rewards | `docs/phase-8-llm-judge.md` | Not started. Runs a full training + evaluation with the judge as the reward, same pipeline/seed/held-out set as Phase 7c's arms, producing numbers that sit directly alongside them. **Scope clarified 2026-07-26**: the paper reports no benchmark result for its judge (Table 2 is all deterministic; the judge appears only as training curves on NQ), so there is no published judge score to check ours against — a limit on external validation, not on the experiment. Must keep the judge reward swappable at the same seam as the deterministic outcome reward so 8b needs no rework |
