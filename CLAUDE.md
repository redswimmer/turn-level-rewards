...
| 8 | LLM-as-judge reward (Bedrock + gpt-oss) — **scope reframed 2026-07-26** | `docs/phase-8-llm-judge.md` | Not started, and **no longer automatically committed work**. Confirmed by direct fetch that the paper never benchmarks its judge (Table 2 is all deterministic; the judge appears only as training curves on NQ), so as originally scoped this phase had no target to reproduce. Reframed as a genuinely original comparison instead: the paper motivates the judge by exact match being "overly rigid", which is the SAME limitation this repo's F1+EM deviation independently addressed — so run three MT-PPO arms (binary EM / LLM judge / F1 partial credit) and ask whether an expensive judge beats a cheap graded metric. Decide after 7c's results. The judge is the paper's separate Appendix C.2/C.3 exploration; building it on top of an incomplete Table 2 reproduction compounds the gap rather than adding to it |
